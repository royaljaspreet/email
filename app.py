...
@app.route('/payment8/<user>',methods = ["POST","GET"])
def payment8(user):
	
	client = razorpay.Client(auth = ("rzp_test_jt1NMWvEInSGIt","wkuS4nu6Kta8ZjoJZN8YNUkZ"))
	payment = client.order.create({ 'amount':500000,'currency':'INR','payment_capture':'1'})
	return render_template('pay.html',payment = payment,user = user,description= "Snow Capped Mountains")		
##----------------------------------------------------------------------------------------------------#
